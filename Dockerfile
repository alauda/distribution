...
COPY --chown=$ALAUDA_UID:$ALAUDA_GID --chmod=640 cmd/registry/config-dev.yml /etc/docker/registry/config.yml
COPY --chown=$ALAUDA_UID:$ALAUDA_GID --chmod=640 cmd/registry/config-alauda.yml /etc/docker/registry/config-alauda.yml
